...
class ToMonthAction extends PeriodJudgment
{
    ...
    {
        $SaleAction = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
            ->where('action_master_id', 39)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();

        $AgreementAction = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
            ->where('action_master_id', 40)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();

        $SettlementAction = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
