...
class ToMonthAction extends PeriodJudgment
{
    ...
    {
        $InCompanyMeeting = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
            ->where('action_master_id', 37)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();

        $Training = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
