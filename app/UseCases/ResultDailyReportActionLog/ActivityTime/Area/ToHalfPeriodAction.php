...
class ToHalfPeriodAction extends PeriodJudgment
{
    ...
    {
        $area_a_count = ResultDailyReportActionLog::whereIn('daily_report_id', $this->Period())
            ->where('action_master_id', 24)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();

        $area_b_count = ResultDailyReportActionLog::whereIn('daily_report_id', $this->Period())
            ->where('action_master_id', 25)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();
        $area_c_count = ResultDailyReportActionLog::whereIn('daily_report_id', $this->Period())
            ->where('action_master_id', 26)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();
        $area_d_count = ResultDailyReportActionLog::whereIn('daily_report_id', $this->Period())
            ->where('action_master_id', 27)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();
        $area_e_count = ResultDailyReportActionLog::whereIn('daily_report_id', $this->Period())
            ->where('action_master_id', 28)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();
        $area_f_count = ResultDailyReportActionLog::whereIn('daily_report_id', $this->Period())
            ->where('action_master_id', 29)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();
        $area_g_count = ResultDailyReportActionLog::whereIn('daily_report_id', $this->Period())
            ->where('action_master_id', 30)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();
        $area_h_count = ResultDailyReportActionLog::whereIn('daily_report_id', $this->Period())
