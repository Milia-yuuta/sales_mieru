...
class ToMonthAction extends PeriodJudgment
{
    ...
    {
        $WebNegotiations = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
            ->where('action_master_id', 34)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();

        $Negotiations = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
            ->where('action_master_id', 35)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();

        $ReNegotiations = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
