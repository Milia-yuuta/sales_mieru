...
class ToMonthAction extends PeriodJudgment
{
    ...
    {
        $PurchasingActivities = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
            ->where('action_master_id', 32)
            ->selectRaw('sum(start_time) as start_time, sum(end_time) as end_time')
            ->first();

        $OfficeWork = ResultDailyReportActionLog::whereIn('daily_report_id', $this->ToMonth())
